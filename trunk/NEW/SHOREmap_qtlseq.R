...
##merge
hs.freq<- rep(1,length(hs[,1]))
ls.freq<- rep(1,length(ls[,1]))
#hs.freq<- apply(hs,1,function(x) x[3]/sum(x[3:5])) #needed for calculation of allele score
#ls.freq<- apply(ls,1,function(x) x[3]/sum(x[3:5])) #needed for calculation of allele
...
# sorted<-t(sapply(o$ix,function(i) c(pos[i],tp[i],aa[i]))) #needed for calculation of allele
 ...
 maxIndex<-which(diff(sign(diff(c(-Inf,pscore.predict,-Inf))))==-2)
 minIndex<-which(diff(sign(diff(c(-Inf,-pscore.predict,-Inf))))==-2)
