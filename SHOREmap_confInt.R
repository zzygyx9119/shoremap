...
ShoreMap.confint <- function(chromosome,positions,background_count,foreground_count, error_count,foreground_frequency=1,level=0.99,minWindow=10){
 data<- cbind(chromosome,positions,foreground_count,background_count,error_count)
 data<- data[rowSums(data[,3:5])>0,]
 #perhaps not the nicest solution
 assign("dataset",data,".GlobalEnv")
 res<- identify_peaks(0,length(data[,2]),foreground_frequency,level,minWindow)
 apply(res,1,function(x) t(c(start=data[x[1],2], stop=data[x[1]+x[2]-1,2],p.value=-1*(x[3]+x[2]))))
}

identify_peaks <- function(indexL,indexH,frequency,level,minWindow){
...
# maxConf(c(beststart,10),level=level,freq=frequency,indexL=indexL,indexH=indexH,minWindow=minWindow)
 ...
 if(res[1,3]<0){
  ...
  resH<- identify_peaks(min(res[1,1]+res[1,2],indexH,length(dataset[,2])),indexH,frequency,level,minWindow)
  if(resL[1,3]<0){
   if(resH[1,3]<0){
    #both good
    rbind(resL,res,resH)
...
loglikelihood_mult <- function(P1=0.5,err=0.01,index=0,size=0){
 ...
 if(P1<0 || P1>1 || err<0 || err>1) {
  ...
  p2<- 1-p1-pe
  p.all <- c(p1,p2,pe)
  -sum(apply(dataset[index:(index+size-1),],1,function(x){dmultinom(x=c(x[3],x[4],x[5]),prob=p.all,log=TRUE)}))
 }
}
...
maxConf<-function(x,level=0.95,freq=0,indexL=0,indexH=Inf,minWindow=10){
 ...
 size<-floor(x[2])
 indexL<- max(0,indexL)
 indexH<- min(length(dataset[,2]),indexH)
 if(size<minWindow){
