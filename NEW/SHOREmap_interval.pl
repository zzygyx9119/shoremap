...
Mandatory:
--target       DOUBLE   Allele frequency of 
--chrsizes     STRING   Chromosome sizes file

Using SHORE's output as input:
--consensus    STRING   SHORE consensus file
--marker       STRING   Marker file (ref & parent or parent1 
                        & parent2)

Using SAMtools' output as input:
...
\n");
        ...
        die(@usage) if (@ARGV == 0);
        GetOptions(\%CMD, "chromosome=s", "begin=s", "end=s", "minfreq=s", "maxfreq=s", "verbose", "parent1=s", "parent2=s", "marker=s", "consensus=s", "chrsizes=s", "windowsize=s", "windowstep=s", "referrors=s", "no_alleles","old-format");
